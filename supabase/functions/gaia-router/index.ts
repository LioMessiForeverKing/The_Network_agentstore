 ...
 */
async function findCandidateAgents(supabase, taskType) {
  try {
 ...
 */
async function executeAgent(supabase, route, taskSpec) {
  try {
    ...
    if (config.invocation_type === 'INTERNAL_FUNCTION') {
      ...
      const functionName = config.function_name || 'atlas-agent';
